...
#include "PRCharacterStat.generated.h"
...
public:
	...
	// 연산자 오버로딩.
	FPRCharacterStat operator+(const FPRCharacterStat& Other) const
	{
		// 현재 데이터 포인터.
		...
		// 결과 저장할 구조체.
		FPRCharacterStat Result;

		// 결과 구조체 포인터.
		float* ResultPtr = reinterpret_cast<float*>(&Result);

		// 구조체에 포함된 float 변수의 개수 확인.
		int32 StatNum = sizeof(FPRCharacterStat) / sizeof(float);
